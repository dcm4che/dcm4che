...
public class LookupTableFactory {
    ...
    private float[] getSharedFunctional(Attributes metadata){
	Sequence ds = metadata.getSequence(Tag.SharedFunctionalGroupsSequence);
	if(ds == null) return null;
	Attributes attribs = ds.get(0);
	ds = attribs.getSequence(Tag.FrameVOILUTSequence);
	attribs = ds.get(0);
	float f1 = attribs.getFloat(Tag.WindowCenter, 0);
	float f2 = attribs.getFloat(Tag.WindowWidth, 0);
	float[] fs = {f1,f2};
	return fs;
    }
	...
	if (preferWindow || vLUT == null) {
		...
		float[] fs = this.getSharedFunctional(img);
		if(fs!=null){
		    windowCenter = fs[0];
		    windowWidth = fs[1];
		}
	...
	if(outBits > 8){
	    ...
	    return new ShortLookupTable(inBits, outBits, 0, lut);
	} else {
	    ...
	    return new ByteLookupTable(inBits, outBits, 0, lut);

	}

    }

    private int calcSigmoid(int outRange, float i, float ic, float k){
