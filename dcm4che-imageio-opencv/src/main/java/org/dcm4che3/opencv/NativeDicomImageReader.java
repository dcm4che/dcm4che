...
public class NativeDicomImageReader extends ImageReader implements Closeable {

    private static final Logger LOGGER = LoggerFactory.getLogger(NativeDicomImageReader.class);
    ...
    public NativeDicomImageReader(ImageReaderSpi originatingProvider) {
    ...
    public void setInput(Object input, boolean seekForwardOnly, boolean ignoreMetadata) {
        ...
        if (input instanceof InputStream) {
            try {
                dis = (input instanceof DicomInputStream) ? (DicomInputStream) input
                    : new DicomInputStream((InputStream) input);
            } catch (IOException e) {
                throw new IllegalArgumentException(e.getMessage());
            }
        } else if (input instanceof DicomMetaData) {
            DicomMetaData m = (DicomMetaData) input;
            initPixelDataFromAttributes(m.getAttributes());
            initPixelDataFile();
            setMetadata(m);
        } else {
    ...
    public ImageInputStream iisOfFrame(int frameIndex) throws IOException {
        ...
        if (epdiis != null) {
            ...
            iisOfFrame = new SegmentedInputImageStream(iis, pixelDataFragments, frameIndex);
        }
    ...
    private PlanarImage getNativeImage(int frameIndex, ImageReadParam param) throws IOException {
        if (dis != null) {
            ...
            if (uri != null && uri.startsWith("file:")) {
                return getNativeImage(new File(uri), frameIndex);
            }
        }
        return null;
    }
    ...
    private PlanarImage getNativeImage(File file, int frame) throws IOException {
        ...
        if (extParams.getSegmentPositions() != null) {
            ...
            if (rawData) {
                MatOfInt dicomparams = new MatOfInt(Imgcodecs.IMREAD_UNCHANGED, dcmFlags, width, height, samples,
                    bitsStored, banded ? Imgcodecs.ILV_NONE : Imgcodecs.ILV_SAMPLE);
                return ImageCV.toImageCV(
                    Imgcodecs.dicomRawRead(file.getAbsolutePath(), positions, lengths, dicomparams, pmi.name()));
            }
    ...
    private ExtendSegmentedInputImageStream buildSegmentedImageInputStream(File file, int frameIndex)
        throws IOException {
        long[] offsets;
        int[] length;
            ...
            if (frames >= nbFragments - 1) {
                ...
                if (frames == 1) {
                    ...
                    if (fragmentsPositions.isEmpty()) {
                        boolean jpeg2000 = getTransferSyntaxUID().startsWith("1.2.840.10008.1.2.4.9");
        ...
        return new ExtendSegmentedInputImageStream(file, offsets, length);
    }
