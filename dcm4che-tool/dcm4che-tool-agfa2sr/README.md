...
Options:
 ...
 -p <pattern>                file path of created DICOM Part 10 files,
                             ...
                             attribute value, e.g.:
                             '{00100020}/{0020000D}.dcm' will store
                             extracted objects using the Study Instance
                             UID (0020,000D) as file name and '.dcm' as
                             file name extension into sub-directories
                             according its Patient ID (0010,0020). At
                             default, extracted objects are stored to the
                             storage directory with the Study Instance UID
                             (0020,000D) as file name without extension.
 -s <[seq.]attr=value>       specify attributes added to created DICOM
                             Part 10 files. attr can be specified by
                             keyword or tag value (in hex).
    --title <code>           Document Title in format (CV, CSD, "CM"),
                             '(18748-4, LN, "Diagnostic Imaging Report")'
                             by default.
 -V,--version                output version information and exit
    --verifying-org <name>   Value of Verifying Organization (0040,A027)
                             in Verifying Observer Sequence (0040,A073) of
                             verified reports, 'N/A' by default.
 -x,--xsl <xsl-file>         apply XSLT stylesheet specified by file path
    ...
    --xml                    disable conversion of output of XSLT into
                             ...
                             '.xml' as file name extension.
    --xml11                  set version in XML declaration to 1.1; 1.0 by
                             default.
Example:
=> agfa2sr agfa_reports.csv
Convert Agfa Reports from file agfa_reports.csv to DICOM SR Documents
files into working directory.
```
