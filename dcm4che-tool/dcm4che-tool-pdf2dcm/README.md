    ...
    Encapsulate PDF (file extension pdf), CDA (file extension xml) or STL
    (file extension stl) into DICOM file. DICOM attributes can be specified
    via command line (using -m option) or a XML file (using -f option). If no
    option is specified, by default they are generated using sample metadata
    file present in the tool. If all the options are specified, system will
